...
const favouriteAdd = async (req, res) => {
  try {
    ...
    const { user_id, product_id, name, price, image_url } = body;

    if (!user_id || !product_id || !name || !price || !image_url) {
      return res.status(400).json({ message: "all required hai" });
    }
...
const favouriteDelete = async (req, res) => {
  try {
    ...
    if (!id) return res.status(400).json({ message: "id required hai" });
